...
1. **Reconstrução COMPLETA do baseline (12 meses × 5 anos) + ativação do fix de EVI2:** o código está pronto e validado por-cena; falta rodar num ambiente com rede estável ao S3 (aqui houve *throttling*). Passos: (a) mover os baselines DN atuais para trás; (b) `build_baseline.py --year-set 2017,2019,2021,2022,2025 --months <N>` mês-a-mês (já força reflectância); (c) **setar `REFLECTANCE_SCALING = True` em `config/settings.py`** para a detecção casar com os novos baselines. Só então o fix de EVI2 fica ativo (acoplamento — F2.1).
