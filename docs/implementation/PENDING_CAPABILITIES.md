# Capacidades aprovadas e ainda não implementadas, e registros de incidente

**Origem:** este arquivo é o conteúdo que estava em `ROADMAP.md` na `main` até
2026-09-08. Ele foi movido para cá quando o Package 2A.6 chegou à `main` e
trouxe consigo **o plano** — que sempre foi um documento diferente com o mesmo
nome, e cuja convivência já custou trabalho duplicado.

**O que este arquivo é:** o registro do que o sistema **não** faz, para que o
código não pareça prometer mais do que entrega, mais os registros de incidente
que não pertencem a nenhum pacote. Nada aqui é plano de execução.

**O que este arquivo NÃO é:** o plano. O plano é
[`../../ROADMAP.md`](../../ROADMAP.md), e desde 2026-09-08 ele está na `main` —
não mais só numa branch de planejamento.

**Por que a separação continua existindo:** um item aqui pode estar aprovado
como tópico do plano, implementado numa branch científica e ainda ausente da
`main`. Essa separação é deliberada: a correção e o sistema em execução se
encontram no replay, não antes. "O defeito está no ar" não significa "a
correção não existe".
