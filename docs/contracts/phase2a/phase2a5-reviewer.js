...
const contextEvidenceById = embedded.context_evidence || {};
const packageBinding = embedded.package_binding || {};
const reviewExportSchema = "https://observatoriodachapadadoararipe.com/data/schemas/phase2a5-review-export-v1.schema.json";
...
const storageBinding = `${packageBinding.package_id}:${packageBinding.review_template_sha256}:${packageBinding.method_evidence_index_sha256}:${packageBinding.context_evidence_index_sha256}`;
const storageKey = `araripe-phase2a5:${reviewerSlot}:${storageBinding}`;
...
function primaryAssessmentComplete(review) {
  return Boolean(
    ...
    && review.contextual_signature.label
    && review.contextual_signature.reason
  );
}
...
function primarySnapshot(review) {
  return structuredClone({
    ...
    contextual_signature: review.contextual_signature,
  });
}
...
function acceptReviewExport(candidate) {
  ...
  if (!sameValue(Object.keys(candidate || {}).sort(), expectedKeys)) {
    throw new Error("The Phase 2A.5 resume envelope has unexpected or missing fields.");
  }
  if (
    candidate.$schema !== reviewExportSchema
    || candidate.schema_version !== "1.0.0"
    || candidate.package_phase !== "phase2a5"
    || candidate.reviewer_slot !== reviewerSlot
    || !Array.isArray(candidate.reviews)
    || !Array.isArray(candidate.reveal_state)
  ) {
    throw new Error("The file belongs to another phase or reviewer slot.");
  }
  if (!sameValue(candidate.package_binding, packageBinding)) {
    throw new Error("The imported review is bound to another package or evidence template.");
  }
  const reviewIds = candidate.reviews.map((item) => item.blind_case_id);
  const revealIds = candidate.reveal_state.map((item) => item?.blind_case_id);
  if (!sameValue(reviewIds, expectedIds) || !sameValue(revealIds, expectedIds)) {
    throw new Error("The imported IDs/order do not match this isolated assignment.");
  }
  const importedRevealed = new Set();
  const importedLocks = new Map();
  candidate.reviews.forEach((review, position) => {
    ...
    if (reveal.revealed) {
      if (!primaryAssessmentComplete(review) || !reveal.locked_primary || !sameValue(primarySnapshot(review), reveal.locked_primary)) {
        throw new Error(`Revealed comparisons do not preserve the locked primary assessment for ${review.blind_case_id}.`);
      }
...
function methodPanel(option, record) {
  ...
  const body = (
    String(record.media_type || "").startsWith("image/")
    || /\\.(png|jpe?g|webp)$/i.test(record.local_path)
  )
    ? `<img src="${escapeHtml(record.local_path)}" alt="Blinded comparison panel ${escapeHtml(option)}">`
    : `<a href="${escapeHtml(record.local_path)}">Open blinded panel ${escapeHtml(option)}</a>`;
  return `<figure class="method-panel"><figcaption><strong>Panel ${escapeHtml(option)}</strong> — ${escapeHtml(record.status)}</figcaption>${body}</figure>`;
}

function familyEvidence(blindCaseId, family) {
  const original = methodEvidenceById[blindCaseId]?.families?.[family];
  const context = contextEvidenceById[blindCaseId]?.families?.[family];
...
function methodSection(currentCase, review) {
  ...
  if (generated && !revealedCases.has(currentCase.blind_case_id)) {
    ...
    return `<section class="method-gate"><p><strong>The interface does not render any of the five blinded comparison families until the primary change, temporal, land-cover, and contextual-signature assessment is saved.</strong></p>
      <button id="reveal-methods" type="button" ${ready ? "" : "disabled"}>Save and lock primary assessment; reveal blinded comparisons</button>
      <p class="meta">${ready ? "The required primary fields are complete." : "Complete every required primary field and reason above before reveal."}</p></section>`;
  }
  return methodFamilies.map((family) => {
    const method = review.method_comparisons[family];
    if (method.availability === "not_generated_in_2a3") {
      return `<section class="method"><strong>${escapeHtml(family)}</strong>: no blinded alternatives were generated.</section>`;
    }
    const evidence = familyEvidence(currentCase.blind_case_id, family);
    const panels = method.display_order.map((option) => methodPanel(option, evidence.options?.[option])).join("");
...
function render() {
  ...
  document.getElementById("app").innerHTML = `<section class="case">
    ...
    <h3>Primary change assessment</h3><div class="form-grid">
    ...
    </div><label>Change reason <textarea id="change-reason" ${primaryLocked}>${escapeHtml(review.change_assessment.reason)}</textarea></label>
    ...
    <h3>Non-causal contextual signature</h3><div class="form-grid">
      <label>Signature <select id="signature-label" ${primaryLocked}>${choices(["fire_like", "exposed_soil_or_clearing_like", "mixed_or_uncertain", "not_assessed"], review.contextual_signature.label)}</select></label>
      <label>Reason <textarea id="signature-reason" ${primaryLocked}>${escapeHtml(review.contextual_signature.reason)}</textarea></label>
    </div>
    <h3>Blinded comparisons</h3><p>Complete and lock the primary assessment first. A preference is evidence for later analysis and never selects or activates an alternative.</p>${methodSection(currentCase, review)}
  ...
  if (reveal) reveal.addEventListener("click", () => {
    ...
    if (!primaryAssessmentComplete(updated)) {
      window.alert("Complete the change, temporal, land-cover, and contextual-signature assessment before reveal.");
      return;
    }
...
function completeEnough(review) {
  ...
  return Boolean(
    ...
    && review.reviewer.qualification_attested
    && review.reviewer.independence_attested
    && primaryAssessmentComplete(review)
...
function saveCurrent({quiet = false} = {}) {
  ...
  if (requestedStatus === "complete" && !completeEnough(review)) {
    ...
    if (!quiet) window.alert("This record is incomplete and was saved as in_progress.");
...
document.getElementById("export").addEventListener("click", () => {
  ...
  link.download = `phase2a5-${reviewerSlot}-reviews.json`;
  link.click();
