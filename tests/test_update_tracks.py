...
# ─── Package 2B.1, preservado no landing do 2A.6 ──────────────────────────────
# O lado científico deste arquivo foi reescrito pelo Package 2A.6 (modelo de
# identidade e linhagem: event_id, identity_kind, origin/split/merge). O teste
# abaixo veio da `main` e NÃO foi reescrito, porque ele não afirma um resultado
# do modelo — ele afirma que `first_seen <= last_seen` **não é invariante**, e
# essa afirmação foi paga com uma parada de produção. Se o modelo determinístico
# tornar o invariante verdadeiro, este teste falha, e a falha é o sinal certo:
# `scripts/r2_state.py` deixou de exigi-lo por causa daquele incidente, e quem
# mudar o comportamento tem de decidir sobre aquela validação também.

def test_uma_aquisicao_fora_de_ordem_e_RECUSADA_pelo_modelo_deterministico():
    """O Package 2A.6 mudou este comportamento, e a mudança tem consequência.

    **O que este teste afirmava antes do landing do 2A.6**, na `main`: que
    `first_seen <= last_seen` **não** é invariante. `update_tracks` fazia
    `w_last[t] = date` incondicionalmente, e cada rodada reprocessava uma janela
    de `SEARCH_DAYS_BACK = 16` dias — então uma rodada posterior que casasse a
    track numa data mais antiga empurrava `last_seen` para trás. Uma validação
    em `scripts/r2_state.py` afirmou o invariante contrário e **derrubou a
    rodada de produção de 2026-09-07** (`feature 101684: first_seen 2026-07-13
    is after last_seen 2026-07-11`), com o estado íntegro.

    **O que mudou:** o estado determinístico do 2A.6 mantém uma marca-d'água e
    **recusa** a aquisição que não é mais nova que ela. O cenário acima deixa de
    produzir `last_seen` para trás porque deixa de ser aceito — a rodada para,
    em vez de reescrever a track. Um preenchimento retroativo passa a exigir
    geração nova, que é a Phase 4.

    **O que NÃO muda, e é o ponto:** `scripts/r2_state.py` continua tolerando
    `first_seen > last_seen` como *oddity* e não como erro. O motivo é outro
    agora: não é mais "o produtor não promete", é **"o produtor promete daqui
    para frente, e o estado que já existe é anterior à promessa"**. As linhas
    que o incidente de 2026-09-07 produziu estão no estado de produção vivo, e
    reapertar aquela checagem quebraria nelas. Quem quiser reapertá-la tem de
    reconstruir o estado primeiro — não é conserto local.
    """
    # rodada A: a cena de 13/07 estabelece a marca-d'água
    _, state = update_tracks(_g([BOX]), None, "2026-07-13")
    assert state.iloc[0]["first_seen"] == state.iloc[0]["last_seen"] == "2026-07-13"

    # rodada B: a mesma janela traz uma cena de 11/07 — agora isto é recusado
    with pytest.raises(OutOfOrderAcquisitionError, match="2026-07-13"):
        update_tracks(_g([BOX]), state, "2026-07-11")

    # e o estado não foi tocado pela tentativa recusada
    assert state.iloc[0]["last_seen"] == "2026-07-13"
