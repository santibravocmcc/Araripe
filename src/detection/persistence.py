...
class PersistenceStateError(PersistenceTransitionError):
    ...
    Inherits `PersistenceTransitionError` so that a caller which already
    refuses a rejected transition (`scripts/run_detection_from_gee.py`) also
    refuses an untrustworthy state, without naming both.
    """


class LegacyPersistenceStateError(PersistenceStateError):
    """The input predates the Phase 2A.1 deterministic state contract.

    A subclass of `PersistenceStateError` on purpose: a previous-generation
    state *is* a state that exists and cannot be trusted for this generation.
    Package 2A.6 and Package 2B.1 arrived at the same fail-closed rule from
    opposite directions — one guarding the schema, the other the counters — and
    conflating them here is what keeps a single loader honest to both.
    """
...
def load_persistence_state(path) -> gpd.GeoDataFrame | None:
    ...
    Três resultados, e a diferença entre eles é o contrato:

    * ``None`` — o arquivo está **genuinamente ausente**. É a primeira execução,
      e ela legitimamente começa com todo alerta como 1ª observação. Este ramo
      vem do Package 2B.1: o chamador não precisa (e não deve) checar
      existência, senão a checagem e a leitura divergem.
    * `PersistenceStateError` — o arquivo **existe e não pode ser lido**. Nunca
      vira estado vazio: nada a jusante distingue um reset de uma onda genuína
      de primeiras observações.
    * `LegacyPersistenceStateError` — o arquivo é de uma **geração anterior**,
      sem os metadados ou as colunas determinísticas do Package 2A.1. Exige
      reconstrução cronológica, que é a Phase 4, não um conserto local.

    As duas últimas são a mesma família por herança, de propósito: ver
    ...
    if not source.exists():
        logger.info(
            "Persistência: nenhum estado em {} — primeira execução", source.name
        )
        return None
    try:
        ...
        raise PersistenceStateError(
            f"não foi possível ler o estado de persistência {source}: {e}"
        ) from e
    if not isinstance(payload, dict) or payload.get("type") != "FeatureCollection":
        raise PersistenceStateError(
            f"estado de persistência {source} não é um FeatureCollection"
        )
    metadata = payload.get(_STATE_METADATA_KEY)
    if not isinstance(metadata, dict):
        raise LegacyPersistenceStateError(
            f"estado de persistência {source} sem os metadados determinísticos "
            f"(colunas exigidas: {', '.join(sorted(_STATE_COLS))}) — "
    ...
    for feature in payload.get("features", []):
        ...
        if not isinstance(properties, dict):
            raise PersistenceStateError(
                f"estado de persistência {source}: feição com properties inválido"
            )
