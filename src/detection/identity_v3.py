"""Deterministic v3 identities for physical acquisitions (Package 2A.6B.1).

This module is deliberately separate from :mod:`src.detection.identity` and
from the audit-only version-2 family.  Version 3 preserves the version-2
semantics unchanged except that platform Sentinel-2C is representable; the
exact delta set is proven by tests/test_v3_structural_equivalence.py.
"""
...
def require_v3_id(value: str, *, prefix: str, label: str) -> str:
...
def sorted_v3_ids(
    ...
    if not items and not allow_empty:
        raise ValueError(f"{label} must contain at least one item")
    for item in items:
        require_v3_id(item, prefix=prefix, label=f"{label} item")
    if len(items) != len(set(items)):
        raise ValueError(f"{label} contains duplicates")
    return tuple(sorted(items, key=lambda item: item.encode("utf-8")))
...
def validate_run_manifest_binding(
    ...
    require_v3_id(run_manifest_id, prefix="run-v3-", label="run_manifest_id")
...
class AcquisitionV3:
    ...
    @classmethod
    def from_dict(cls, value: dict[str, Any]) -> "AcquisitionV3":
        if value.get("schema_version") != SCHEMA_VERSION:
            raise IdentityMajorError("acquisition must use schema_version 3.0.0")
        created = create_acquisition_v3(
        ...
        if value != created.to_dict():
            raise ValueError(
                "acquisition payload is non-canonical or does not match its v3 inputs"
            )
        return created


def create_acquisition_v3(
    *,
    run_manifest_id: str,
    run_manifest_sha256: str,
    collection_id: str,
    ...
    return AcquisitionV3(
        acquisition_id="acq-v3-" + digest,
...
def acquisition_order_key(acquisition: AcquisitionV3) -> tuple[datetime, str]:
...
class ObservationV3:
    ...
    @classmethod
    def from_dict(
        cls, value: dict[str, Any], *, acquisition: AcquisitionV3
    ) -> "ObservationV3":
        if value.get("schema_version") != SCHEMA_VERSION:
            raise IdentityMajorError("observation must use schema_version 3.0.0")
        ...
        for field in required_equal:
            if value.get(field) != created.value[field]:
                raise ValueError(f"observation {field} does not match its v3 inputs")
        if value != created.to_dict():
            raise ValueError(
                "observation payload is non-canonical or does not match its v3 inputs"
            )
        return created


def create_observation_v3(
    *,
    ...
    baseline_version = require_semver(baseline_version, label="baseline_version")
    observation_id_value = observation_id_v3(
    ...
    return ObservationV3(
        {
            ...
            "identity_inputs_sha256": observation_id_value.removeprefix("obs-v3-"),
...
def validate_observation_v3(value: dict[str, Any]) -> None:
    ...
    if value.get("identity_inputs_sha256") != expected_id.removeprefix("obs-v3-"):
    ...
    if value.get("raw_detection_policy") != {
        ...
        raise ValueError("observation raw-detection policy differs from v3")
...
def origin_event_id_v3(first_observation_id: str) -> str:
    require_v3_id(
        first_observation_id, prefix="obs-v3-", label="first_observation_id"
    )
    return "evt-v3-" + identity_sha256(
        "event-v3", "origin", first_observation_id
    )


def child_event_id_v3(
    operation: str,
    ...
    parents = sorted_v3_ids(
        parent_event_ids, prefix="evt-v3-", label="parent_event_ids"
    )
    triggers = sorted_v3_ids(
...
def lineage_id_v3(
    *,
    ...
    children = sorted_v3_ids(
        child_event_ids, prefix="evt-v3-", label="child_event_ids"
    )
    triggers = sorted_v3_ids(
        trigger_observation_ids,
        prefix="obs-v3-",
    ...
    if relation == "split":
        expected_children = sorted_v3_ids(
            (
                child_event_id_v3("split", parents, [trigger])
    ...
    if relation == "merge":
        expected_child = child_event_id_v3("merge", parents, triggers)
    ...
    return "lin-v3-" + digest


def contribution_key_v3(event_id: str, observed_on: str) -> str:
    require_v3_id(event_id, prefix="evt-v3-", label="event_id")
    observed_on = require_utc_date(observed_on)
    return "pc-v3-" + identity_sha256(
        "persistence-contribution-v3", event_id, observed_on
    )
