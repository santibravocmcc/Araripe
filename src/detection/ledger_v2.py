...
def _expected_record(acquisition: AcquisitionV2) -> dict[str, Any]:
...
class ProcessingLedgerV2:
    ...
    def __init__(
        ...
        *,
        ...
        acquisitions: Iterable[AcquisitionV2],
        ...
        self.ledger_id = "pl-v2-" + identity_digest
        self.identity_inputs_sha256 = identity_digest
        self._rows: dict[str, dict[str, Any]] = {}

    @property
    def acquisitions(self) -> tuple[AcquisitionV2, ...]:
    ...
    def expected_for_date(self, observed_on: str) -> tuple[AcquisitionV2, ...]:
        return tuple(
            item
            for item in self._acquisitions
    ...
    def record_terminal(
        ...
        *,
        ...
        if status not in TERMINAL_STATUSES:
            raise ValueError("status is not a processing-ledger-v2 terminal status")
        observations = sorted_v2_ids(
            observation_ids,
            prefix="obs-v2-",
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        validate_processing_ledger_v2(document)
        return document

    def to_bytes(self) -> bytes:
        return serialize_v2_document("processing-ledger-v2", self.to_dict())
...
def validate_processing_ledger_v2(payload: dict[str, Any]) -> None:
    """Rebuild and compare a ledger to enforce all semantic invariants."""

    validate_v2_schema("processing-ledger-v2", payload)
    try:
        acquisitions = tuple(
            AcquisitionV2.from_dict(item)
            for item in payload["expected_acquisitions"]
        )
        rebuilt = ProcessingLedgerV2(
        ...
        raise ContractV2ValidationError(
            "processing-ledger-v2 semantic validation failed"
        ) from exc
    if payload != canonical:
        raise ContractV2ValidationError(
            "processing-ledger-v2 hashes, order, or reconciliation differ "
