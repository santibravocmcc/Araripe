"""Package 2A.6B.1 v3 run producer: physical acquisitions plus terminal evidence.

Consumes — never redefines — the Package 2A.6B.1 v3 contract family:
``AcquisitionV3``/``ObservationV3`` from :mod:`src.detection.identity_v3` and
``ProcessingLedgerV3`` from :mod:`src.detection.ledger_v3`.  For one run
manifest it:

- turns each declared physical datatake (platform + datatake + timestamp and
  its provider-native scenes) into exactly one ``AcquisitionV3`` bound to
...
Whether a normalized platform is representable is decided by the
acquisition-v3 contract (S2A, S2B, and S2C, per the 2026-09-01 amendment);
this module still adds no platform policy of its own, so a later unreviewed
unit such as Sentinel-2D fails closed at that contract.
"""
...
def _declared_input(value: DatatakeRunInputV3) -> DatatakeRunInputV3:
    if not isinstance(value, DatatakeRunInputV3):
        raise TypeError("datatakes must be DatatakeRunInputV3 records")
...
class CompositionRunV3:
    ...
    created_at: str
    datatakes: tuple[DatatakeRunInputV3, ...]
    reviewed_baselines: tuple[str, ...] = REVIEWED_PROCESSING_BASELINES
    ledger: ProcessingLedgerV3 = field(init=False)
    outcomes: tuple[CompositionOutcomeV3, ...] = field(init=False)
    ...
    def __post_init__(self) -> None:
        ...
        self.ledger = ProcessingLedgerV3(
            run_manifest_id=self.run_manifest_id,
            run_manifest_sha256=self.run_manifest_sha256,
    ...
    def compose_all(self, *, terminal_at: str) -> tuple[CompositionOutcomeV3, ...]:
    ...
    def _composable_outcome(
        self, outcome: CompositionOutcomeV3
    ) -> CompositionOutcomeV3:
    ...
    def record_complete(
        self,
        outcome: CompositionOutcomeV3,
        *,
        observations: Iterable[ObservationV3],
        terminal_at: str,
    ) -> Mapping[str, Any]:
        ...
        for observation in observation_records:
            if not isinstance(observation, ObservationV3):
                raise TypeError("observations must be ObservationV3 records")
    ...
    def record_zero_valid(
        self, outcome: CompositionOutcomeV3, *, terminal_at: str
    ) -> Mapping[str, Any]:
