"""Atomic per-UTC-date persistence transitions for Package 2A.6A.
...
class PersistenceV2Error(RuntimeError):
    """Base class for a rejected v2 state transition."""


class StateGenerationMismatchError(PersistenceV2Error):
    """The state and runtime generation bindings differ."""


class OutOfOrderRequiresRebuild(PersistenceV2Error):
    """An older unfinalized date cannot mutate the current generation."""


class LateArrivalRequiresRebuild(PersistenceV2Error):
    """A finalized date has a different manifest or input digest."""


class ObservationLedgerMismatch(PersistenceV2Error):
    """Raw observations do not reconcile with terminal ledger rows."""


class AmbiguousLineageError(PersistenceV2Error):
    """One acquisition creates an unsupported many-to-many component."""


@dataclass(frozen=True)
class PersistenceTransitionV2:
    state: dict[str, Any]
    outcome: str
    state_changed: bool
...
def _require_generation_id(value: str) -> str:
    return require_v2_id(value, prefix="gen-v2-", label="generation_id")
...
def _event_identity_basis_digest(event_id: str) -> str:
    ...
    return event_id.removeprefix("evt-v2-")
...
def _lineage_record(
    *,
    ...
    return {
        ...
        "identity_inputs_sha256": lineage_id.removeprefix("lin-v2-"),
...
def _seal_state(state: dict[str, Any]) -> dict[str, Any]:
    ...
    sealed["state_id"] = "state-v2-" + digest
...
def empty_persistence_state_v2(
    *,
    generation_id: str,
...
def _validate_state_binding(
    state: dict[str, Any], ledger: ProcessingLedgerV2, observed_on: str
) -> None:
    validate_persistence_state_v2(state)
    if state.get("schema_version") != SCHEMA_VERSION:
        raise IdentityMajorError("persistence state must use schema_version 2.0.0")
    require_v2_id(state.get("state_id"), prefix="state-v2-", label="state_id")
...
def _normalize_observations(
    observations: Iterable[ObservationV2 | dict[str, Any]],
    *,
    ledger: ProcessingLedgerV2,
    observed_on: str,
    baseline_version: str,
) -> tuple[list[dict[str, Any]], dict[str, AcquisitionV2]]:
    expected = {
    ...
    for item in observations:
        raw = item.to_dict() if isinstance(item, ObservationV2) else deepcopy(item)
        ...
        validated = ObservationV2.from_dict(raw, acquisition=acquisition).to_dict()
...
def _reconcile_observations_with_ledger(
    *,
    ...
    expected: dict[str, AcquisitionV2],
    ledger: ProcessingLedgerV2,
    observed_on: str,
) -> None:
...
def _process_acquisition(
    *,
    ...
    for parent_id in sorted(
        ...
        child_ids = [
            child_event_id_v2("split", [parent_id], [trigger_id])
    ...
    for index, observation in enumerate(observations):
        ...
        if not parents:
            event_id = origin_event_id_v2(observation["observation_id"])
...
def _contribution_record(
    *,
    ...
    key = contribution_key_v2(event_id, observed_on)
    return {
        "schema_version": SCHEMA_VERSION,
        "contribution_key": key,
        "identity_inputs_sha256": key.removeprefix("pc-v2-"),
        "run_manifest_id": ledger.run_manifest_id,
        "run_manifest_sha256": ledger.run_manifest_sha256,
        "event_id": event_id,
...
def apply_terminal_date_v2(
    *,
        ...
        for contribution in state["contributions"]:
            ...
            for observation_id in contribution["source_observation_ids"]:
                assignments[observation_id] = {
                    ...
                    "persistence_tier": persistence_tier_v2(historical_count),
                }
        return PersistenceTransitionV2(
            state=state,
    ...
    for event_id in sorted(event_sources, key=lambda item: item.encode("utf-8")):
        ...
        event["persistence_tier"] = persistence_tier_v2(
...
def rebuild_persistence_state_v2(
    *,
    generation_id: str,
    ledger: ProcessingLedgerV2,
    observations: Iterable[ObservationV2 | dict[str, Any]],
    ...
    records = [
        item.to_dict() if isinstance(item, ObservationV2) else deepcopy(item)
        for item in observations
    ]
...
def persistence_state_bytes_v2(state: dict[str, Any]) -> bytes:
    validate_persistence_state_v2(state)
    return serialize_v2_document("persistence-state-v2", state)
...
def _require_canonical_values(values: list[str], *, label: str) -> None:
    if values != sorted(set(values), key=lambda item: item.encode("utf-8")):
        raise ContractV2ValidationError(f"{label} must be unique and UTF-8 sorted")


def _validate_event_identity_v2(event: dict[str, Any]) -> None:
    validate_run_manifest_binding(
        event["run_manifest_id"], event["run_manifest_sha256"]
    )
    event_id = require_v2_id(event["event_id"], prefix="evt-v2-", label="event_id")
    ...
    if kind == "origin":
        ...
        expected_id = origin_event_id_v2(first)
    elif kind in {"split", "merge"}:
        if basis["first_observation_id"] is not None:
            raise ContractV2ValidationError(
                "derived event cannot have a first observation"
            )
        expected_id = child_event_id_v2(kind, parents, triggers)
    else:
        raise ContractV2ValidationError("unknown event identity kind")
    if event_id != expected_id:
        raise ContractV2ValidationError("event identity mismatch")
    if event["identity_inputs_sha256"] != event_id.removeprefix("evt-v2-"):
        raise ContractV2ValidationError("event identity-input checksum mismatch")

    for field, prefix in (
        ("observation_ids", "obs-v2-"),
        ("acquisition_ids", "acq-v2-"),
        ("contribution_keys", "pc-v2-"),
        ("incoming_lineage_ids", "lin-v2-"),
        ("outgoing_lineage_ids", "lin-v2-"),
    ):
        values = event[field]
        _require_canonical_values(values, label=f"event {field}")
        for value in values:
            require_v2_id(value, prefix=prefix, label=f"event {field} item")
    ...
    if event["n_distinct_observation_dates"] != len(dates):
        raise ContractV2ValidationError("event distinct-date count mismatch")
    if dates and (
        event["first_observed_on"] != dates[0]
        or event["last_observed_on"] != dates[-1]
    ):
        raise ContractV2ValidationError("event first/last dates are not reconciled")
    ...
    if event["representative_geometry"] != canonical_geometry:
        raise ContractV2ValidationError("event geometry is not canonical")
    if event["representative_geometry_sha256"] != geometry_digest:
        raise ContractV2ValidationError("event geometry checksum mismatch")
    if event["raw_observations_preserved"] is not True:
        raise ContractV2ValidationError("event does not preserve raw observations")
    for field in ("created_at", "updated_at"):
        if event[field] != normalize_utc_timestamp(event[field], label=field):
            raise ContractV2ValidationError(f"event {field} is non-canonical")


def validate_event_v2(event: dict[str, Any]) -> None:
    validate_v2_schema("event-v2", event)
    _validate_event_identity_v2(event)
    if "persistence_tier" in event:
        expected_tier = persistence_tier_v2(
            event["n_distinct_observation_dates"]
        )
        if event["persistence_tier"] != expected_tier:
            raise ContractV2ValidationError("event persistence tier mismatch")


def validate_lineage_v2(edge: dict[str, Any]) -> None:
    validate_v2_schema("lineage-v2", edge)
    validate_run_manifest_binding(edge["run_manifest_id"], edge["run_manifest_sha256"])
    for field, prefix in (
        ("parent_event_ids", "evt-v2-"),
        ("child_event_ids", "evt-v2-"),
        ("trigger_observation_ids", "obs-v2-"),
    ):
        _require_canonical_values(edge[field], label=f"lineage {field}")
        for value in edge[field]:
            require_v2_id(value, prefix=prefix, label=f"lineage {field} item")
    expected_id = lineage_id_v2(
    ...
    if edge["lineage_id"] != expected_id:
        raise ContractV2ValidationError("lineage identity mismatch")
    if edge["identity_inputs_sha256"] != expected_id.removeprefix("lin-v2-"):
        raise ContractV2ValidationError("lineage identity-input checksum mismatch")
    ...
    if edge["observed_on"] != edge["effective_timestamp_utc"][:10]:
        raise ContractV2ValidationError("lineage UTC date differs from its timestamp")
    if edge["created_at"] != normalize_utc_timestamp(
        edge["created_at"], label="created_at"
    ):
        raise ContractV2ValidationError("lineage created_at is non-canonical")


def validate_persistence_contribution_v2(contribution: dict[str, Any]) -> None:
    validate_v2_schema("persistence-contribution-v2", contribution)
    validate_run_manifest_binding(
        contribution["run_manifest_id"], contribution["run_manifest_sha256"]
    )
    for field, prefix in (
        ("source_acquisition_ids", "acq-v2-"),
        ("source_observation_ids", "obs-v2-"),
    ):
        _require_canonical_values(contribution[field], label=f"contribution {field}")
        for value in contribution[field]:
            require_v2_id(value, prefix=prefix, label=f"contribution {field} item")
    expected_key = contribution_key_v2(
        contribution["event_id"], contribution["observed_on"]
    )
    if contribution["contribution_key"] != expected_key:
        raise ContractV2ValidationError("contribution identity mismatch")
    if contribution["identity_inputs_sha256"] != expected_key.removeprefix("pc-v2-"):
        raise ContractV2ValidationError(
    ...
    if contribution["finalized_at"] != normalize_utc_timestamp(
        ...
        raise ContractV2ValidationError("contribution finalized_at is non-canonical")


def validate_persistence_state_v2(state: dict[str, Any]) -> None:
    """Validate schema, hashes, ordering, references, and date uniqueness."""

    validate_v2_schema("persistence-state-v2", state)
    if state["generated_at"] != normalize_utc_timestamp(
        state["generated_at"], label="generated_at"
    ):
        raise ContractV2ValidationError("state generated_at is non-canonical")
    expected_digest = _state_digest(*_state_identity_inputs(state))
    if state["state_id"] != "state-v2-" + expected_digest:
        raise ContractV2ValidationError("persistence state_id checksum mismatch")
    ...
    for field, value in components.items():
        if integrity[field] != canonical_sha256(value):
            raise ContractV2ValidationError(f"persistence {field} mismatch")
    ...
    if integrity["document_sha256"] != canonical_sha256(
        ...
        raise ContractV2ValidationError("persistence document checksum mismatch")

    finalized_dates = state["finalized_dates"]
    if finalized_dates != sorted(
        finalized_dates, key=lambda item: item["observed_on"]
    ):
        raise ContractV2ValidationError("finalized dates are not ordered")
    if len({item["observed_on"] for item in finalized_dates}) != len(
        finalized_dates
    ):
        raise ContractV2ValidationError("duplicate finalized UTC date")
    events = state["events"]
    lineage = state["lineage"]
    contributions = state["contributions"]
    if events != sorted(events, key=lambda item: item["event_id"].encode("utf-8")):
        raise ContractV2ValidationError("events are not event-ID ordered")
    if lineage != sorted(
        lineage, key=lambda item: item["lineage_id"].encode("utf-8")
    ):
        raise ContractV2ValidationError("lineage is not lineage-ID ordered")
    if contributions != sorted(
        contributions,
        key=lambda item: item["contribution_key"].encode("utf-8"),
    ):
        raise ContractV2ValidationError("contributions are not key ordered")
    ...
    if len(event_ids) != len(events):
        raise ContractV2ValidationError("duplicate event ID in persistence state")
    if len(lineage_ids) != len(lineage):
        raise ContractV2ValidationError("duplicate lineage ID in persistence state")
    ...
    if state["transition_policy"] != {
        ...
        raise ContractV2ValidationError("persistence transition policy mismatch")
    if finalized_dates:
        ...
        raise ContractV2ValidationError("empty state cannot have a watermark")
    else:
        expected_last_transition = {
            "outcome": "initialized",
            "observed_on": None,
    ...
    for contribution in contributions:
        validate_persistence_contribution_v2(contribution)
        expected_key = contribution["contribution_key"]
        if contribution["event_id"] not in event_ids:
            raise ContractV2ValidationError("contribution references unknown event")
    ...
    for event in events:
        ...
        expected_tier = persistence_tier_v2(len(expected_event_dates))
        if event["persistence_tier"] != expected_tier:
            raise ContractV2ValidationError("event persistence tier mismatch")
        ...
        if not set(event["incoming_lineage_ids"]) <= lineage_ids:
            raise ContractV2ValidationError("event references unknown incoming lineage")
        if not set(event["outgoing_lineage_ids"]) <= lineage_ids:
            raise ContractV2ValidationError("event references unknown outgoing lineage")
    ...
    for edge in lineage:
        ...
        if not set(edge["parent_event_ids"]) <= event_ids:
            raise ContractV2ValidationError("lineage references unknown parent event")
        if not set(edge["child_event_ids"]) <= event_ids:
            raise ContractV2ValidationError("lineage references unknown child event")
...
def load_persistence_state_v2(path) -> dict[str, Any]:
    state = load_v2_document(path, "persistence-state-v2")
    validate_persistence_state_v2(state)
    return state
